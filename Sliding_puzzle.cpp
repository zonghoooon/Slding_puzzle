...
pn solve(pn tnode) {
    ...
    do {
        ...
        if (node.empty[0] != X - 1 && node.from != 'D') {
        ...
        if (node.empty[1] != Y - 1 && node.from != 'R') {
...
pn find_value(pn node) {
    ...
    for (int i = 0; i < X; i++) {
        for (int j = 0; j < Y; j++) {
            val = node.puzzle[i][j];
            if (val == 0) {
                node.value += (X + Y - 2) - i - j;
                node.empty[0] = i;
                node.empty[1] = j;

            }
            else {
                node.value += abs((val - 1) / Y - i) + abs((val - 1) % Y - j);
            }
        }
    } //맨하탄거리 이용
...
pn find_value2(pn node, char from) {
    ...
    case 'L':
        ...
        node.value = pre_value + abs((val - 1) / Y - node.empty[0]) + abs((val - 1) % Y - node.empty[1]);
        node.empty[1]++;
        node.value -= abs((val - 1) / Y - node.empty[0]) + abs((val - 1) % Y - node.empty[1]);
        node.value--;
        break;
    case 'R':
        val = node.puzzle[node.empty[0]][node.empty[1]];
...
bool solvable(pn node) {
    ...
    for (int i = 0; i < X * Y; i++) {
        for (int j = i + 1; j < X * Y; j++) {
            if (node.puzzle[i / Y][i % Y] > node.puzzle[j / Y][j % Y] && node.puzzle[j / Y][j % Y] != 0) {
                inv++;
            }
        }
    }
